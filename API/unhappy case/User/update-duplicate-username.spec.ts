...
test.describe('Update User - Duplicate Username', () => {
    let token = '';
    let userId = '';
    ...
    test.beforeAll(async ({ request, baseURL }) => {
        ...
        // Get second user's username (existing username)
        existingUsername = users[1].username;
        
        expect(userId).toBeDefined();
        expect(existingUsername).toBeDefined();
        console.log(`Selected user ID: ${userId}`);
        console.log(`Existing username to test duplicate: ${existingUsername}`);
    });

    test('Should fail to update user with duplicate username', async ({ request, baseURL }) => {
        ...
        console.log('Duplicate Username Error Response:', JSON.stringify(body, null, 2));
